        ...
        try:
            ...
            while True:
                try:
                    ...
                    com = com + ' :] ' + comments[i]['message'].encode('utf8', errors= 'replace').decode('utf8')
                    i = i + 1
                except:
                    break
                    ############
...
def processFacebookPageFeedStatus(status):
    ...
    status_message = ' ' if 'message' not in status.keys() else status['message'].encode('utf8', errors='replace').decode('utf8')
...
def write_csv(wan_data, num):
    with open('data %s %s.csv' % (since, until), 'wt', encoding = 'utf8') as file:  #, encoding='utf8'
        w = csv.writer(file)
